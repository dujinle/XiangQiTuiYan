cc.Class({
	...
	onLoad(){
		...
		this.node.on("PRGameStatus", this.gameStatusFunc, this);
		this.clearGame();
	...
	startGame(){
		...
		if(gBoardGame.isGameOver == 0){
			...
			//游戏开始 棋盘棋子添加点击事件
			this.qipan.getComponent("RjdzQpNode").setOtherNodePressActive(0,true);
		}else{
			//停止游戏
			this.setGameButtonsActive(false);
			this.startButton.getChildByName("start").active = true;
			this.startButton.getChildByName("stop").active = false;
			this.qipan.getComponent("RjdzQpNode").setOtherNodePressActive(0,false);
			gBoardGame.isGameOver = 0;
		}
	},
	onExit(){
		gBoardGame.ClearBoard();
		cc.eventManager.removeListener(this.qipan.getComponent("RjdzQpNode").touchListener);
	...
	gameStatusFunc(event){
		...
		if(this.qipan.getComponent("RjdzQpNode").gameStatus == 3){
			this.PGameStatus.getComponent("PopResult").setStatus("你赢了！");
		}else if(this.qipan.getComponent("RjdzQpNode").gameStatus == 6){
			this.PGameStatus.getComponent("PopResult").setStatus("你输了！");
		}else if(this.qipan.getComponent("RjdzQpNode").gameStatus == 7){

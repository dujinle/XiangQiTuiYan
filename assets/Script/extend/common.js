...
* my    当前移动的棋子角色
*/
g_com.bylaw.z = function(x,y,nx,ny,map,my){
...
g_com.bylaw.p = function(x,y,nx,ny,map,my){
	/*必须是直来直往*/
	if((nx - x) != 0 && (ny - y) != 0){
		return -1;
	}
...
g_com.bylaw.c = function(x,y,nx,ny,map,my){
	/*必须是直来直往*/
	if((nx - x) != 0 && (ny - y) != 0){
		return -1;
	}
...
g_com.bylaw.m = function(x,y,nx,ny,map,my){
	/*判断是否走得日字*/
...
g_com.bylaw.x = function(x,y,nx,ny,map,my){
	/*判断是否走得田字*/
	if(Math.abs(y - ny) != 2){
		return -1;
	}
...
g_com.bylaw.s = function(x,y,nx,ny,map,my){
	var dist = (x - nx) * (x - nx) + (y - ny) * (y - ny);
	//移动距离为1 不然就返回原来的位置
	if(dist != 2){
		return -1;
...
g_com.bylaw.j = function(x,y,nx,ny,map,my){
	var dist = Math.abs(x - nx) + Math.abs(y - ny);
	//移动距离为1 不然就返回原来的位置
	if(dist != 1){
		return -1;

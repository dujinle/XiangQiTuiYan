cc.Class({
    ...
    onLoad () {
		...
		g_root_node_com.chooser_cb = function(data){
			self.onFileOpenResult(data);
		};
	...
	onFileOpenResult(data){
		var self = this;
		this.file_path = data;
		cc.loader.load(this.file_path, function (err,texture) {
			if(err == null){
				var sprite = self.label.getComponent(cc.Sprite);
